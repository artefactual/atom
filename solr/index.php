...
<body>
  <section class="container">
    ...
    <?php
      if ('POST' === $_SERVER['REQUEST_METHOD'] and isset($_POST['index'])) {
          indexData($solrClient);
      }

      if ('POST' === $_SERVER['REQUEST_METHOD'] and isset($_POST['delete'])) {
          deleteData($solrClient);
      }

      if ('POST' === $_SERVER['REQUEST_METHOD'] and isset($_POST['reindex'])) {
          deleteData($solrClient);
          indexData($solrClient);
      }

      if ('POST' === $_SERVER['REQUEST_METHOD'] and isset($_POST['search']) and isset($_POST['query'])) {
          searchQuery($solrClient, $_POST['query']);
      }

      if ('POST' === $_SERVER['REQUEST_METHOD'] and isset($_POST['init'])) {
          initializeCollection();
      }

      if ('POST' === $_SERVER['REQUEST_METHOD'] and isset($_POST['addCopy'])) {
      ...
      function indexData($client)
      {
          ...
          if ($conn->connect_error) {
              exit("<p class='info info--error'>Connection failed: ".$conn->connect_error.'</p>');
          }
