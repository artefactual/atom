(function ($) {
  QubitDialog = function (table, options) {
    ...
    var matches = $(":input[name]:first", thisDialog.table)
      .attr("name")
      .match(this.fieldNameFilter);
    if (null != matches) {
    ...
    var addNewText =
      this.options.addNewText === undefined ? "Add new" : options.addNewText;
    ...
    $("select[id=relatedAuthorityRecord_type]", thisDialog.table).change(
      function () {
        var subTypeField = $(
    ...
    var handleYuiSubmit = function () {
      ...
      if (
        "undefined" !== typeof thisDialog.options.validator &&
    ...
    var submitText =
      this.options.submitText === undefined ? "Submit" : options.submitText;
    var cancelText =
      this.options.cancelText === undefined ? "Cancel" : options.cancelText;
    this.yuiDialog = new YAHOO.widget.Dialog($yuiDialogWrapper[0], {
      buttons: [
        { text: submitText, handler: handleYuiSubmit, isDefault: true },
    ...
    this.yuiDialog.cfg.queueProperty(
      "keylisteners",
    ...
    this.renderField = function (fname) {
      if ($(this.fields[fname]).next().hasClass("form-autocomplete")) {
        ...
        "select-one" === this.fields[fname].type ||
    ...
    this.getField = function (fname) {
      if (
        null != this.fieldPrefix &&
        null == fname.match(this.fieldNameFilter)
      ) {
    ...
    this.updateDialog = function (thisId, thisData, callback) {
      ...
      for (fieldname in this.fields) {
        ...
        if (
          ...
          var displayField =
            fieldname.substr(0, fieldname.length - 1) + "Display]";
          if (undefined !== thisData[displayField]) {
            $(hiddenInput)
              .next("input")
              .val(jQuery.trim(thisData[displayField]));
    ...
    this.save = function (yuiDialogData) {
      $("input.form-autocomplete", thisDialog.table).each(function () {
        ...
        for (var i in thisDialog.iframes) {
          if (
            thisDialog.id === thisDialog.iframes[i].dialogId &&
        ...
        if (0 < this.value.length) {
          ...
          // Remove <iframe/> if selecting a pre-existing value
            ...
            undefined !== index &&
            this.value !== thisDialog.iframes[index].iframe.value
          ) {
      ...
      for (var id in this.data) {
        ...
        for (var j in thisData) {
          ...
          for (var k in this.iframes) {
            if (
              id === this.iframes[k].dialogId &&
              j === this.iframes[k].inputName
            ) {
              this.iframes[k].iframe.one(
                "load",
                { hdn: $hidden },
                function (e) {
