  <did>
    ...
    <?php foreach($objects as $object): ?>
      <container type="<?php echo $object->type ?>">
        <?php echo esc_specialchars($object->location) ?>
        <?php if($object->name): ?>
          <title><?php echo esc_specialchars($object->name) ?></title>
        <?php endif; ?>
      </container>
    <?php endforeach; ?>
    ...
    <?php if (0 < count($notes = $$resourceVar->getNotesByType(array('noteTypeId' => QubitTerm::GENERAL_NOTE_ID)))): ?>
      <?php foreach ($notes as $note): ?>
        <note type="<?php echo esc_specialchars($note->getType(array('cultureFallback' => true))) ?>" encodinganalog="3.6.1"><p><?php echo esc_specialchars($note->getContent(array('cultureFallback' => true))) ?></p></note>
      <?php endforeach; ?>
    <?php endif; ?>
