...
class QubitSolrAclSearch
{
     ...
     */
    public static function filterByRepository(arSolrPluginQuery $query, $action)
    {
     ...
     * @return \Elastica\Query Filtered query
     */
    public static function filterByResource(arSolrPluginQuery $query, $root)
    {
     ...
     * Filter search query by resource specific ACL.
     *
     * @param arSolrBoolQuery $queryBool Search query object
     */
    public static function filterDrafts(arSolrBoolQuery $queryBool)
    {
        ...
        if (1 == count($repositoryViewDrafts)) {
            ...
            while ($repo = array_shift($repositoryViewDrafts)) {
                $query->addShould(new arSolrTermQuery(['repository.id' => (int) $repo['id']]));
            }

            $query->addShould(new arSolrTermQuery(['publicationStatusId' => QubitTerm::PUBLICATION_STATUS_PUBLISHED_ID]));
