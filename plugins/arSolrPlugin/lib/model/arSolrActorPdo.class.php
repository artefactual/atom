...
class arSolrActorPdo
{
    ...
    public function serialize()
    {
        ...
        foreach (QubitPdo::fetchAll($sql, [$this->id, QubitTerm::MAINTENANCE_NOTE_ID]) as $item) {
            $serialized['maintenanceNotes'][] = arSolrNote::serialize($item);
        }
        ...
        $serialized['i18n'] = arSolrModelBase::serializeI18ns($this->id, ['QubitActor']);
    ...
    public function serializeAltNames()
    {
        ...
        foreach (QubitPdo::fetchAll($sql, [$this->id, QubitTerm::STANDARDIZED_FORM_OF_NAME_ID]) as $item) {
            $serialized['standardizedNames'][] = arSolrOtherName::serialize($item);
        }
