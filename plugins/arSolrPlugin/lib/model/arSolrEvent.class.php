...
class arSolrEvent extends arSolrModelBase
{
    public static function serialize($event)
    {
        ...
        if (isset($event->start_date)) {
            $serialized['startDate'] = arSolrPluginUtil::normalizeDateWithoutMonthOrDay($event->start_date);
            $serialized['startDateString'] = Qubit::renderDate($event->start_date);
        }

        if (isset($event->end_date)) {
            $serialized['endDate'] = arSolrPluginUtil::normalizeDateWithoutMonthOrDay($event->end_date, true);
