 ...
 */
class arSolrInformationObjectPdo
{
    public $ancestors;
    public $doc;
    public $repository;
    ...
    public function serialize()
    {
        ...
        // Dates
        foreach ($this->events as $event) {
            $serialized['dates'][] = arSolrEvent::serialize($event);
            ...
            if (!empty($event->start_date)) {
                $serialized['startDateSort'] = arSolrPluginUtil::normalizeDateWithoutMonthOrDay($event->start_date);
            }

            if (!empty($event->end_date)) {
                $serialized['endDateSort'] = arSolrPluginUtil::normalizeDateWithoutMonthOrDay($event->end_date, true);
            }
        }
        ...
        if (null !== $repository = $this->getRepository()) {
            ...
            $serialized['repository']['i18n'] = arSolrModelBase::serializeI18ns(
                $repository->id,
                ['QubitActor'],
        ...
        if (isset($relatedTerms[QubitTaxonomy::PLACE_ID])) {
            ...
            $extendedPlaceIds = arSolrModelBase::extendRelatedTerms(
                $relatedTerms[QubitTaxonomy::PLACE_ID]
            );

            foreach ($extendedPlaceIds as $id) {
                $node = new arSolrTermPdo($id);

                $places = [
                    'id' => $node->id,
                    'i18n' => arSolrModelBase::serializeI18ns(
                        $node->id,
                        ['QubitTerm'],
                        ['fields' => ['name']]
                    ),
        ...
        if (isset($relatedTerms[QubitTaxonomy::SUBJECT_ID])) {
            ...
            $extendedSubjectIds = arSolrModelBase::extendRelatedTerms(
        ...
        if (isset($relatedTerms[QubitTaxonomy::GENRE_ID])) {
            ...
            $extendedGenreIds = arSolrModelBase::extendRelatedTerms(
        ...
        foreach ($this->getNotesByType(QubitTerm::GENERAL_NOTE_ID) as $item) {
            $serialized['generalNotes'][] = arSolrNote::serialize($item);
        }

        // PREMIS data
        if (null !== $premisData = arSolrPluginUtil::getPremisData($this->id, self::$conn)) {
        ...
        $serialized['createdAt'] = arSolrPluginUtil::convertDate($this->created_at);
        $serialized['updatedAt'] = arSolrPluginUtil::convertDate($this->updated_at);

        $serialized['sourceCulture'] = $this->source_culture;
        $serialized['i18n'] = arSolrModelBase::serializeI18ns($this->id, ['QubitInformationObject']);
        ...
        if (count($this->ancestors) > 1) {
            ...
            $i18nFields = arSolrModelBase::serializeI18ns(
                $collectionRootId,
    ...
    private function getBasisRights()
    {
        ...
        foreach ($this->getRights() as $right) {
            ...
            $basisRight['startDate'] = arSolrPluginUtil::normalizeDateWithoutMonthOrDay($right->start_date);
            $basisRight['endDate'] = arSolrPluginUtil::normalizeDateWithoutMonthOrDay($right->end_date, true);
    ...
    private function getActRights()
    {
        ...
        foreach ($this->getGrantedRights() as $grantedRight) {
            ...
            $actRight['startDate'] = arSolrPluginUtil::normalizeDateWithoutMonthOrDay($grantedRight->start_date);
            $actRight['endDate'] = arSolrPluginUtil::normalizeDateWithoutMonthOrDay($grantedRight->end_date, true);
