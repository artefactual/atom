...
class arSolrAccession extends arSolrModelBase
{
    ...
    public static function getAccessionEvents($accessionId)
    {
        ...
        foreach (self::$statements['event']->fetchAll(PDO::FETCH_ASSOC) as $item) {
            ...
            $event['date'] = arSolrPluginUtil::normalizeDateWithoutMonthOrDay($item['date']);
            $event['dateString'] = Qubit::renderDate($item['date']);

            // Serialize type term
            if ($item['type_id']) {
                $event['type'] = ['i18n' => arSolrModelBase::serializeI18ns(
                    $item['type_id'],
                    ['QubitTerm'],
                    ['fields' => ['name']]
                )];
            ...
            // Serialize note
            if ($item['note_id']) {
                $event['notes'] = ['i18n' => arSolrModelBase::serializeI18ns(
                    $item['note_id'],
                    ['QubitNote'],
                    ['fields' => ['content']]
                )];
            ...
            $event['i18n'] = arSolrModelBase::serializeI18ns($item['id'], ['QubitAccessionEvent']);
    ...
    private static function serialize($id)
    {
        ...
        $serialized['date'] = arSolrPluginUtil::convertDate($data['date']);
        $serialized['createdAt'] = arSolrPluginUtil::convertDate($data['created_at']);
        $serialized['updatedAt'] = arSolrPluginUtil::convertDate($data['updated_at']);
