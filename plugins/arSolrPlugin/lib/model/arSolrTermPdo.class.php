...
class arSolrTermPdo
{
    ...
    public function serialize()
    {
        ...
        foreach (QubitPdo::fetchAll($sql, [$this->id, QubitTerm::ALTERNATIVE_LABEL_ID]) as $item) {
            $serialized['useFor'][] = arSolrOtherName::serialize($item);
        }
        ...
        foreach (QubitPdo::fetchAll($sql, [$this->id, QubitTerm::SCOPE_NOTE_ID]) as $item) {
            $serialized['scopeNotes'][] = arSolrNote::serialize($item);
        }
        ...
        $serialized['i18n'] = arSolrModelBase::serializeI18ns($this->id, ['QubitTerm']);
