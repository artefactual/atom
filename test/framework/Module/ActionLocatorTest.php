<?php

/*
 * This file is part of the Access to Memory (AtoM) software.
 *
 * Access to Memory (AtoM) is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * Access to Memory (AtoM) is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with Access to Memory (AtoM). If not, see <http://www.gnu.org/licenses/>.
 */

declare(strict_types=1);

namespace Atom\Tests\Framework\Module;

use Atom\Framework\Module\ActionDescriptor;
use Atom\Framework\Module\ActionLocator;
use Atom\Framework\Module\ModuleDirectories;
use Atom\Framework\Module\ModuleException;
use PHPUnit\Framework\TestCase;

/**
 * @internal
 */
final class ActionLocatorTest extends TestCase
{
    public function testFindsApplicationActions(): void
    {
        $projectDirectory = dirname(__DIR__, 3);
        $locator = new ActionLocator(new ModuleDirectories(
            $projectDirectory,
            'qubit',
        ));

        self::assertEquals(new ActionDescriptor(
            'informationobject',
            'index',
            'informationobjectindexAction',
            $projectDirectory
                .'/apps/qubit/modules/informationobject/actions/'
                .'indexAction.class.php',
        ), $locator->find('informationobject', 'index'));
    }

    public function testFindsPluginActionsBeforeApplicationActions(): void
    {
        $projectDirectory = dirname(__DIR__, 3);
        $locator = new ActionLocator(new ModuleDirectories(
            $projectDirectory,
            'qubit',
            ['arOidcPlugin'],
        ));

        self::assertEquals(new ActionDescriptor(
            'user',
            'edit',
            'usereditAction',
            $projectDirectory
                .'/plugins/arOidcPlugin/modules/user/actions/'
                .'editAction.class.php',
        ), $locator->find('user', 'edit'));
    }

    public function testReturnsNullForUnknownAction(): void
    {
        $locator = new ActionLocator(new ModuleDirectories(
            dirname(__DIR__, 3),
            'qubit',
        ));

        self::assertNull($locator->find('informationobject', 'notPresent'));
    }

    public function testRejectsUnsafeNames(): void
    {
        $locator = new ActionLocator(new ModuleDirectories(
            dirname(__DIR__, 3),
            'qubit',
        ));

        $this->expectException(ModuleException::class);
        $locator->find('../config', 'secrets');
    }
}
