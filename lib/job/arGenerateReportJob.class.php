...
{
    public const itemOrFileTemplatePath = 'apps/qubit/modules/informationobject/templates/itemOrFileListSuccess.php';
    public const storageLocationsTemplatePath = 'apps/qubit/modules/informationobject/templates/storageLocationsSuccess.php';
    public const boxLabelTemplatePath = 'apps/qubit/modules/informationobject/templates/boxLabelSuccess.php';
    public const reportsDir = 'downloads/reports';
    /**
    ...
    public function runJob($parameters)
    {
        ...
        if (null === $this->resource = QubitInformationObject::getById($this->params['objectId'])) {
            $this->error($this->i18n->__(
                'Error: Could not find an information object with id: %1',
    ...
    private function getFileOrItemListResults($levelOfDescription)
    {
        ...
        foreach ($ios as $item) {
            ...
            $results[$parentFile][] = [
                ...
                'dates' => isset($creationDates) ? Qubit::renderDateStartEnd(
                    $creationDates->getDate(['cultureFallback' => true]),
                    $creationDates->startDate,
                    $creationDates->endDate
                ) : '',
