...
/**
 * Render a standard AtoM form field.
 *
 * Renders the field label, field specific error text, translation source text,
 * and the form field (input, select, etc.) with the current field value.
 *
 * @param sfFormField $field    Form field object
 * @param mixed       $resource The main ORM object for this form
 * @param array       $options  Optional parameters
 */
function render_field($field, $resource = null, array $options = [])
{
    $sourceHelper = render_source_helper(
        $field->getName(), $resource, $options
    );

    if (sfConfig::get('app_b5_theme', false)) {
        return render_b5_field($field, $sourceHelper, $options);
    }

    if (isset($options['onlyInput']) && $options['onlyInput']) {
        $field = $sourceHelper.$field->render($options);
    } else {
...
/**
 * Render a div showing this field's "source_culture" value, to aid translation.
 *
 * @param string $fieldname Form field name
 * @param mixed  $resource  The main ORM object for this form
 * @param array  $options   Optional parameters
 */
function render_source_helper($fieldname, $resource = null, array $options = [])
{
    // Don't show source text if this field always uses the "source_culture"
    // value
    ...
    // Don't show source text if creating a new resource, or if the current
    // culture is the same as the resource source_culture
    ...
    try {
        $source = $resourceRaw->__get($fieldname, ['sourceCulture' => true]);
        $fallback = $resourceRaw->__get($fieldname);
    } catch (Exception $e) {
    ...
    // Don't show helper if there is no source text, or if the field already
    // has a value in the current culture
    ...
    if (
        sfCultureInfo::getInstance($culture)->direction
        != $sourceCultureInfo->direction
    ) {
