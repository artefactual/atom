...
class QubitRequestToPublish extends BaseRequestToPublish
{
  public function __toString()
  {
    ...
    if (!isset($string)) {
      $string = $this->getRTP_name(['sourceCulture' => true]);
    }
  ...
  {
    if (!isset($this->slug)) {
      $this->slug = QubitSlug::slugify($this->__get('rtp_name', ['sourceCulture' => true]));
    }
  ...
  public function getLabel()
  {
    ...
    if (0 == strlen($rtp_name = $this->getRTP_name())) {
      $rtp_name = $this->getRTP(['sourceCulture' => true]);
    }
    ...
    if (0 == strlen($rtp_email = $this->getRTP_email())) {
      $rtp_email = $this->getRTP_email(['sourceCulture' => true]);
    }
    ...
    if (0 == strlen($rtp_relationship = $this->getRTP_motivation())) {
      $rtp_relationship = $this->getRTP_motivation(['sourceCulture' => true]);
    }
  ...
  public function deleteInformationObjectRelations()
  {
    ...
    ['requestorId' => QubitTaxonomy::REQUEST_TO_PUBLISH_ID]);
