...
abstract class BaseClipboardSaveItem implements ArrayAccess
{
  const
    DATABASE_NAME = 'propel',

    TABLE_NAME = 'clipboard_save_item',

    ID = 'clipboard_save_item.ID',
    SAVE_ID = 'clipboard_save_item.SAVE_ID',
    ITEM_CLASS_NAME = 'clipboard_save_item.ITEM_CLASS_NAME',
    SLUG = 'clipboard_save_item.SLUG';

  public static function addSelectColumns(Criteria $criteria)
  {
    $criteria->addSelectColumn(QubitClipboardSaveItem::ID);
    $criteria->addSelectColumn(QubitClipboardSaveItem::SAVE_ID);
    $criteria->addSelectColumn(QubitClipboardSaveItem::ITEM_CLASS_NAME);
    $criteria->addSelectColumn(QubitClipboardSaveItem::SLUG);

    return $criteria;
  }

  protected static
    $clipboardSaveItems = array();
  ...
  public static function getFromRow(array $row)
  {
    ...
    $key = serialize($keys);
    if (!isset(self::$clipboardSaveItems[$key]))
    {
      $clipboardSaveItem = new QubitClipboardSaveItem;

      $clipboardSaveItem->keys = $keys;
      $clipboardSaveItem->row = $row;

      $clipboardSaveItem->new = false;

      self::$clipboardSaveItems[$key] = $clipboardSaveItem;
    }

    return self::$clipboardSaveItems[$key];
  }

  public static function clearCache()
  {
    self::$clipboardSaveItems = array();
  }
  ...
  public static function get(Criteria $criteria, array $options = array())
  {
    ...
    {
      $options['connection'] = Propel::getConnection(QubitClipboardSaveItem::DATABASE_NAME);
    }

    self::addSelectColumns($criteria);

    return QubitQuery::createFromCriteria($criteria, 'QubitClipboardSaveItem', $options);
  }
  ...
  public static function getById($id, array $options = array())
  {
    ...
    $criteria->add(QubitClipboardSaveItem::ID, $id);
  ...
  {
    $this->tables[] = Propel::getDatabaseMap(QubitClipboardSaveItem::DATABASE_NAME)->getTable(QubitClipboardSaveItem::TABLE_NAME);
  }
  ...
  protected function rowOffsetGet($name, $offset, $options)
  {
    ...
    if (!array_key_exists($offset, $this->row))
    {
      ...
      $criteria = new Criteria;
      $criteria->add(QubitClipboardSaveItem::ID, $this->id);
  ...
  public static function addJoinsaveCriteria(Criteria $criteria)
  {
    $criteria->addJoin(QubitClipboardSaveItem::SAVE_ID, QubitClipboardSave::ID);
