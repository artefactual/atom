...
abstract class BaseClipboardSave implements ArrayAccess
{
  const
    DATABASE_NAME = 'propel',

    TABLE_NAME = 'clipboard_save',

    ID = 'clipboard_save.ID',
    USER_ID = 'clipboard_save.USER_ID',
    PASSWORD = 'clipboard_save.PASSWORD',
    CREATED_AT = 'clipboard_save.CREATED_AT';

  public static function addSelectColumns(Criteria $criteria)
  {
    $criteria->addSelectColumn(QubitClipboardSave::ID);
    $criteria->addSelectColumn(QubitClipboardSave::USER_ID);
    $criteria->addSelectColumn(QubitClipboardSave::PASSWORD);
    $criteria->addSelectColumn(QubitClipboardSave::CREATED_AT);

    return $criteria;
  }

  protected static
    $clipboardSaves = array();
  ...
  public static function getFromRow(array $row)
  {
    ...
    $key = serialize($keys);
    if (!isset(self::$clipboardSaves[$key]))
    {
      $clipboardSave = new QubitClipboardSave;

      $clipboardSave->keys = $keys;
      $clipboardSave->row = $row;

      $clipboardSave->new = false;

      self::$clipboardSaves[$key] = $clipboardSave;
    }

    return self::$clipboardSaves[$key];
  }

  public static function clearCache()
  {
    self::$clipboardSaves = array();
  }
  ...
  public static function get(Criteria $criteria, array $options = array())
  {
    ...
    {
      $options['connection'] = Propel::getConnection(QubitClipboardSave::DATABASE_NAME);
    }

    self::addSelectColumns($criteria);

    return QubitQuery::createFromCriteria($criteria, 'QubitClipboardSave', $options);
  }
  ...
  public static function getById($id, array $options = array())
  {
    ...
    $criteria->add(QubitClipboardSave::ID, $id);
  ...
  {
    $this->tables[] = Propel::getDatabaseMap(QubitClipboardSave::DATABASE_NAME)->getTable(QubitClipboardSave::TABLE_NAME);
  }
  ...
  protected function rowOffsetGet($name, $offset, $options)
  {
    ...
    if (!array_key_exists($offset, $this->row))
    {
      ...
      $criteria = new Criteria;
      $criteria->add(QubitClipboardSave::ID, $this->id);
  ...
  {
    $criteria->addJoin(QubitClipboardSave::USER_ID, QubitUser::ID);
  ...
  {
    $criteria->add(QubitClipboardSaveItem::SAVE_ID, $id);
