...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink"
    ...
    <xsl:template name="titleNotes">
        <fo:block xsl:use-attribute-sets="smp">
            <xsl:if test="ead:odd">
                <fo:block xsl:use-attribute-sets="h3ID">Notes du titre</fo:block>
    ...
    <xsl:template match="ead:titlestmt" mode="coverPage">
        ...
        <fo:block border-bottom="1pt solid #666" margin-top="0in" id="cover-page">
            <fo:block xsl:use-attribute-sets="h1" linefeed-treatment="preserve">
                <xsl:choose>
                    <xsl:when test="ead:titleproper[@type='filing']">
                        Instrument de recherche - <xsl:apply-templates select="ead:titleproper[@type='filing']"/>
                    </xsl:when>
                    <xsl:otherwise>
                        ...
                        <xsl:apply-templates select="(//ead:repository/ead:corpname)[1]"/>
                        <fo:block/>
                        Instrument de recherche - <xsl:apply-templates select="ead:titleproper[1]"/> (<xsl:value-of select="//ead:eadid"/>)
                    </xsl:otherwise>
                </xsl:choose>
            </fo:block>
    ...
    <xsl:template match="ead:profiledesc/ead:creation">
        Généré par les
        <xsl:apply-templates
            select="/ead:ead/ead:eadheader/ead:filedesc/ead:publicationstmt/ead:publisher"
            mode="coverPage"
        />
        le <xsl:apply-templates select="ead:date"/>
    </xsl:template>
    ...
    <xsl:template match="ead:archdesc" mode="bookmarks">
        <fo:bookmark-tree>
            <fo:bookmark internal-destination="cover-page">
                <fo:bookmark-title>Page de titre</fo:bookmark-title>
            </fo:bookmark>
            <xsl:if test="ead:did">
    ...
    <xsl:template match="ead:archdesc" mode="toc">
        <fo:block line-height="18pt" margin-top="0.25in">
            <fo:block xsl:use-attribute-sets="h2" id="toc">Table des matières</fo:block>
    ...
    <xsl:template match="ead:archdesc/ead:did">
        <fo:block xsl:use-attribute-sets="section">
        <fo:block xsl:use-attribute-sets="h2ID">Information sommaire</fo:block>
            <!--
            ...
            <fo:table table-layout="fixed" width="100%">
                ...
                <fo:table-body>
                        ...
                        <xsl:call-template name="summaryInfoOtherField">
                            ...
                            <xsl:with-param name="label" select="'Dates de production, de révision et de suppression'"/>
                        </xsl:call-template>
                    ...
                    <xsl:if test="self::ead:unitdate">
                        <xsl:choose>
                            <xsl:when test="@datechar = 'creation'">
                                <xsl:text> (date(s) de création)</xsl:text>
                            </xsl:when>
    ...
    <xsl:template name="otherNotes">
        <xsl:if test="ead:odd">
            <fo:block xsl:use-attribute-sets="h3ID">Autres notes</fo:block>
            <fo:list-block xsl:use-attribute-sets="section">
                <xsl:for-each select="ead:odd">
                    <xsl:variable name="otherNoteHeading">
                        <xsl:if test="current()[@type='levelOfDetail']">Niveau de détail</xsl:if>
                        <xsl:if test="current()[@type='statusDescription']">Statut de la description</xsl:if>
                        <xsl:if test="current()[@type='descriptionIdentifier']">Identifiant de la description</xsl:if>
                        <xsl:if test="current()[@type='institutionIdentifier']">Identifiant du service responsable de la description</xsl:if>
                        <xsl:if test="current()[@type='edition']">Édition</xsl:if>
                        <xsl:if test="current()[@type='physDesc']">Description matérielle</xsl:if>
                        <xsl:if test="current()[@type='conservation']">Restauration / Conservation</xsl:if>
                        <xsl:if test="current()[@type='material']">Documents d’accompagnement</xsl:if>
                        <xsl:if test="current()[@type='alphanumericDesignation']">Indications alpha-numériques</xsl:if>
                        <xsl:if test="current()[@type='bibSeries']">Note sur la collection</xsl:if>
                        <xsl:if test="current()[@type='rights']">Droits</xsl:if>
                        <xsl:if test="current()[@type='general']">Note générale</xsl:if>
                        <xsl:if test="current()[@type='publicationStatus']">Statut de la notice</xsl:if>
                    </xsl:variable>
        ...
        <fo:list-item>
            ...
            <fo:list-item-body start-indent="body-start()">
                <fo:block>
                    ...
                    <xsl:if test="name()='genreform'"> (dénomination générale des documents)</xsl:if>
                    <xsl:if test="name()='subject'"> (sujet)</xsl:if>
                    <xsl:if test="name()='geogname'"> (place)</xsl:if>
                    <xsl:if test="name()='name' or name()='persname'">
                        <xsl:if test="current()[@role='Creator']"> (créateur)</xsl:if>
                        <xsl:if test="current()[@role='subject']"> (sujet)</xsl:if>
                    </xsl:if>
                </fo:block>
            </fo:list-item-body>
        </fo:list-item>
   ...
   <!-- Linking elmenets -->
    ...
    <xsl:template name="tableHeaders">
        <fo:table-row background-color="#f7f7f9" padding-left="2pt" margin-left="2pt">
            <fo:table-cell><fo:block>Cote</fo:block></fo:table-cell>
            <fo:table-cell><fo:block>Titre</fo:block></fo:table-cell>
            <fo:table-cell><fo:block>Dates</fo:block></fo:table-cell>
            <fo:table-cell><fo:block>Unité de conditionnement</fo:block></fo:table-cell>
        </fo:table-row>
    </xsl:template>
    ...
    <xsl:template match="ead:did" mode="dscSeriesTitle">
        <fo:block font-weight="bold" font-size="14" margin-bottom="0" margin-top="0" id="{local:buildID(parent::*)}">
            ...
            <xsl:if test="ead:unitid">
                <xsl:choose>
                    <xsl:when test="../@level='series'">Série <xsl:value-of select="ead:unitid"/>: </xsl:when>
                    <xsl:when test="../@level='subseries'">Sous-série <xsl:value-of select="ead:unitid"/>: </xsl:when>
                    <xsl:when test="../@level='subsubseries'">Sous-sous-série <xsl:value-of select="ead:unitid"/>: </xsl:when>
                    ...
                    <xsl:when test="../@level='subfonds'">Sous-fonds <xsl:value-of select="ead:unitid"/>: </xsl:when>
                    <xsl:when test="../@level='recordgrp'">Groupe d'enregistrements <xsl:value-of select="ead:unitid"/>: </xsl:when>
                    <xsl:when test="../@level='subgrp'">Sous-groupe <xsl:value-of select="ead:unitid"/>: </xsl:when>
                    <xsl:otherwise>
    ...
    <xsl:template match="ead:repository | ead:origination | ead:unitdate | ead:unitid | ead:scopecontent
        ...
        <fo:block xsl:use-attribute-sets="smpDsc">
            ...
            <xsl:if test="name()='unitdate'"> (date(s) de création)</xsl:if>
        </fo:block>
    </xsl:template>
