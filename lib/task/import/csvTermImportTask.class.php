    ...
    public function execute($arguments = [], $options = [])
    {
        ...
        $import = new QubitFlatfileImport([
            ...
            'saveLogic' => function (&$self) {
                ...
                $culture = (!empty($self->columnValue('culture'))) ? $self->columnValue('culture') : 'en';
                ...
                // Note parent ID if it's set
                $parentId = null;
                if ($self->columnExists('parentId') && !empty($self->columnExists('parentId'))) {
                ...
                // Take note of named parent
                if ($self->columnExists('parentName')) {
                    // Initialize parent names data for taxonomy/term/culture
                    if (empty($self->status['parentNames'])) {
                        $self->status['parentNames'] = [];
                    }

                    if (empty($self->status['parentNames'][$term->taxonomyId])) {
                        $self->status['parentNames'][$term->taxonomyId] = [];
                    }

                    if (empty($self->status['parentNames'][$term->taxonomyId][$term->id])) {
                        $self->status['parentNames'][$term->taxonomyId][$term->id] = [];
                    }

                    // Store taxonomy/term ID/culture of parent name
                    $self->status['parentNames'][$term->taxonomyId][$term->id][$culture] = $self->columnValue('parentName');
                }
            ...
            'completeLogic' => function (&$self) {
                ...
                // Set parents by name
                if (isset($self->status["parentNames"])) {
                    foreach ($self->status["parentNames"] as $taxonomyId => $terms) {
                        foreach ($terms as $termId => $cultures) {
                            foreach ($cultures as $culture => $name) {
                                ...
                                $parentTerm = QubitTerm::getOne($criteria);
