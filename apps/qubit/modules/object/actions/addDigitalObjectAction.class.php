...
class ObjectAddDigitalObjectAction extends sfAction
{
     ...
     *
     * @param mixed $metadataCollection
     */
    private function applyMetadataToInformationObject($metadataCollection)
    {
        ...
        if (
            isset($metadataCollection['exif']['gps_latitude'], $metadataCollection['exif']['gps_longitude'])
        ) {
    ...
    private function extractComprehensiveMetadata($filePath)
    {
        ...
        if (isset($meta['GPSLatitude'], $meta['GPSLongitude'])) {
    ...
    private function generateTitleFromExif($metadataCollection)
    {
        ...
        if (isset($exif['camera_make'], $exif['camera_model'])) {
            $titleParts[] =
                'Photo taken with '.
    ...
    private function generateDescriptionFromExif($metadataCollection)
    {
        ...
        // Image properties
        if (isset($exif['width'], $exif['height'])) {
            $descriptionParts[] =
                'Image dimensions: '.
                $exif['width'].
                ' × '.
        ...
        if (isset($exif['gps_latitude'], $exif['gps_longitude'])) {
            $descriptionParts[] =
                'Geographic location: '.
                $exif['gps_latitude'].
                ', '.
    ...
    private function generateCreatorFromExif($metadataCollection)
    {
        ...
        if (isset($exif['camera_make'], $exif['camera_model'])) {
            ...
            if (
                false !== stripos($deviceName, 'huawei')
                || false !== stripos($deviceName, 'samsung')
                || false !== stripos($deviceName, 'iphone')
                || false !== stripos($deviceName, 'pixel')
            ) {
            ...
            if (
                false !== stripos($deviceName, 'canon')
                || false !== stripos($deviceName, 'nikon')
                || false !== stripos($deviceName, 'sony')
                || false !== stripos($deviceName, 'olympus')
            ) {
    ...
    private function generateKeywordsFromExif($metadataCollection)
    {
        ...
        if (isset($exif['focal_length'], $exif['aperture'])) {
            $focalLength = (float) $exif['focal_length'];
            $aperture = (float) $exif['aperture'];
    ...
    private function addComprehensiveMetadata($metadataCollection)
    {
        try {
            ...
            if (
                $currentPhysical
                && false !== strpos($currentPhysical, 'Technical Metadata:')
            ) {
                $currentPhysical = preg_replace(
