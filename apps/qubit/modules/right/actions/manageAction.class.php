...
class RightManageAction extends sfAction
{
  ...
  protected function formSetup()
  {
    ...
    $this->form->setWidget('all_or_digital_only', new sfWidgetFormChoice(array(
      'expanded' => true,
      'choices'  => array('all' => 'Apply to all descendants', 'digital_only' => 'Apply only to digital object descendants'),
      'default'  => 'all'
    )));

    $this->form->setWidget('overwrite_or_combine', new sfWidgetFormChoice(array(
      'expanded' => true,
      'choices'  => $choices = array('overwrite' => 'Overwrite existing rights', 'combine' => 'Add to existing rights'),
      'default'  => 'overwrite'
    )));
  ...
  public function execute($request)
  {
    ...
    if ($request->isMethod('post'))
    {
      ...
      if ($this->form->isValid())
      {
        ...
        // que job with params
        QubitJob::runJob('arInheritRightsJob', $jobParams);
