...
class sfObjectRoute extends sfRequestRoute
{
  ...
  protected function convertObjectToArray($params)
  {
    if (!$this->compiled)
    {
      $this->compile();
    ...
    if (!is_array($params))
    {
      return $this->doConvertObjectToArray($params);
    }

    foreach (array('sf_subject', 0) as $key)
    {
      if (isset($params[$key]))
      {
        $params += $this->doConvertObjectToArray($params[$key]);
        unset($params[$key]);
