...
class sfRoute implements Serializable
{
  ...
  public function matchesParameters($params, $context = array())
  {
    ...
    if (!$this->options['extra_parameters_as_query_string'] && false === strpos($this->regex, '<_star>') && array_diff_key($params, $this->variables, $defaults))
    {
      return false;
    }
    ...
    if (array_diff_assoc(array_diff_key($this->params, $this->variables), $tparams))
    {
      return false;
    }
  ...
  protected function tokenize()
  {
    ...
    while (strlen($buffer))
    {
      ...
      else if (preg_match('#^'.$this->options['variable_prefix_regex'].'('.$this->options['variable_regex'].')#', $buffer, $match))
      {
        // a variable
      ...
      else if (preg_match('#^('.$this->options['text_regex'].')(?:'.$this->options['segment_separators_regex'].'|'.$this->options['variable_prefix_regex'].$this->options['variable_regex'].'|$)#', $buffer, $match))
      {
        // a text
