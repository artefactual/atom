...
class sfCultureInfo
{
  ...
  public function __get($name)
  {
    ...
    if (in_array(strtolower($getProperty), $this->properties))
    {
  ...
  {
    $this->properties = array_map('strtolower', get_class_methods($this));
  ...
  /**
   * Gets the direction of the current culture, whether it is left to right or right to left.
   *
   * @return string either ltr for left to right or rtl for rith to left
   */
  public function getDirection()
  {
    if (in_array(substr($this->culture, 0, 2), array('ar', 'fa', 'he', 'ps', 'ur')))
    {
      return 'rtl';
    }
