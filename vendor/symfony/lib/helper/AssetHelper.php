...
function get_stylesheets()
{
  ...
  // if the current culture is right to left, add stylesheets with right to
  // left overrides, if they exist
  ...
  foreach ($response->getStylesheets() as $file => $options)
  {
    ...
    if ('rtl' == $cultureInfo->direction)
    {
      ...
      $path = sfConfig::get('sf_web_dir').$path.'.css';

      if (file_exists($path))
      {
        $html .= stylesheet_tag($name, $options);
      }
    }
  }
